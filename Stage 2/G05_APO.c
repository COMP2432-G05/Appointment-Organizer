            ...
            while((n = read(p2c[i][0], p2cBuf, BUF_SIZE)) > 0){
                ...
                while(flag){
                    ...
                    if(strcmp(p2cBuf,"cancel")==0){
                        ...
                        write(c2p[i][1],c2pBuf,BUF_SIZE);
                        read(p2c[i][0], p2cBuf,BUF_SIZE);
                        strcpy(c2pBuf,get_ultilization_rate(name[i],pTimetable,days));
                        write(c2p[i][1],c2pBuf,BUF_SIZE);//send the utilization rate
                        break;
                    }
                        ...
                        if(av==1){/*add the code for available*/
                            ...
                            reject_list[n]='\0';
                            write(c2p[i][1],"remove",BUF_SIZE);
                            write(c2p[i][1],reject_list,BUF_SIZE);//rejct list will be send
                        }
                        else{
                            write(c2p[i][1],"n",BUF_SIZE);
                        }
            ...
            //------------------------[PARENT] DISPATCHING APPOINTMENTS TO USERS----------------------
                ...
                while(second!=NULL){
                    if(second->able==true){
                        ...
                        request_reject++;
                        sprintf(temp_buff,"%s%.0f. %s\n",temp_buff,request_reject,second->sentence);
                    }
                    second=second->next;
                }
            ...
            if(strcmp(token, "PRIORITY") == 0 || strcmp(token,"ALL")==0){
                ...
                while(second!=NULL){
                    ...
                    for(index=0; index<second->all_num; index++){
                        ...
                        else if(strcmp(c2pBuf,"remove")==0&&flag2){
                            flag=true;
                            int n;
                            n=read(c2p[ID][0],c2pBuf,BUF_SIZE);//the id of the meeting to be reject
                            sprintf(Reject,"%s %s",Reject,c2pBuf);
                        }
                        else if(strcmp(c2pBuf,"remove")==0){//avoid the message not been read
                            read(c2p[ID][0],c2pBuf,BUF_SIZE);
                        }
                    }
                    if(flag){//to remove all the meeting
                        int index_p;
                        char temp[BUF_SIZE];
                        strcpy(temp,Reject);
                        printf("%s\n",temp);
                        char *tok=strtok(Reject," ");
                        while(tok!=NULL){
